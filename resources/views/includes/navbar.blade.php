    ...
    <div class=" mx-auto px-4" @click.away="showMenu = false" x-data="{ showMenu: false }">
        <div class="flex justify-between">
            <div class="flex space-x-4 lg:space-x-7 w-full justify-between items-center">
                ...
                <div class=" bg-gray-100 lg:bg-transparent top-[4.5rem] rounded-lg lg:rounded-none px-2 lg:px-0 w-44 lg:w-max flex flex-col lg:flex-row lg:items-center space-x-0 space-y-2 lg:space-y-0 lg:space-x-1 fixed lg:relative lg:top-0 right-2   transition ease-linear duration-300"
                    x-bind:class="showMenu ? 'translate-x-0' : 'translate-x-44 lg:translate-x-0'">
                    <a href="{{ route('home') }}"
                        class="py-4 px-2 font-semibold hover:text-primary transition duration-300  {{ request()->routeIs('home') ? ' text-primary border-b-4 border-primary font-semibold' : 'text-gray-500' }}  ">Home</a>
                    ...
                    <form action="/auth/logout">
                        <button type="submit"
                        class="py-4 px-2 font-semibold hover:text-primary transition duration-300 {{ request()->routeIs('about') ? ' text-primary border-b-4 border-primary font-semibold' : 'text-red-600' }}">Salir</button>
                    </form>
                    @else
                    <livewire:auth.login /> 
