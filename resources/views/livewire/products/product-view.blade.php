<div class="flex flex-col space-y-4">
    ...
    @php
        $cant = count($products);
        $randProd = $cant ? $products[rand(0, $cant - 1)] : null;
        $randProd = json_decode(json_encode($randProd));
    @endphp
    <!-- Jumbotron -->
    <div class="text-center max-w-[64rem] mx-auto w-full relative overflow-hidden bg-center bg-no-repeat bg-cover rounded-lg"
        style="background-image: url({{ $randProd ? $randProd->image : env('LOGO_URL') }}); height: 400px;">
        @if ($randProd)
            <div class="absolute top-0 right-0 bottom-0 left-0 w-full h-full overflow-hidden bg-fixed"
                ...
                <div class="flex flex-col space-y-2 justify-center items-center h-full">
                    <div class="text-white">
                        ...
                        <h4 class="font-semibold text-xl lg:text-2xl mb-6">{{ $randProd->name }} -
                            {{ $randProd->stock->formatted_price }}</h4>
                        <a class="inline-block px-7 py-3 mb-1 border-2 border-gray-200 text-gray-200 font-medium text-sm leading-snug uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out"
                            href="{{ route('products.index', ['search' => $randProd->name]) }}" role="button"
                            data-mdb-ripple="true" data-mdb-ripple-color="light">Explora Otros Productos</a>
                    </div>
                </div>
            </div>
        @endif
    ...
    <div
        class="grid w-full max-w-xs md:max-w-4xl lg:max-w-7xl xl:max-w-none mx-auto grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-2 md:gap-4 lg:gap-6">
